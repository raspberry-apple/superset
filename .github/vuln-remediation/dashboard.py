...
def generate_markdown_report(
    ...
    durations = [t.duration_seconds for t in succeeded + failed + timed_out if t.duration_seconds]
    ...
    lines.extend([
        ...
        "|-----------|----------------|------------|",
        f"| Python (pip) | {len(python_tasks)} | {sum(1 for t in python_tasks if t.status == SessionStatus.SUCCEEDED)} |",
        f"| JavaScript (npm) | {len(npm_tasks)} | {sum(1 for t in npm_tasks if t.status == SessionStatus.SUCCEEDED)} |",
        "",
        "---",
        "",
    ])
    ...
    for i, task in enumerate(tasks, 1):
        ...
        lines.append(
            f"| {i} | `{v.package}` | `{v.cve_id}` | {v.severity.value.upper()} "
            f"| {status_emoji} {task.status.value} | {issue_link} | {pr_link} | {session_link} |"
        )
    ...
    lines.extend([
        ...
        "",
        "---",
        f"_Generated at {timestamp} by [vuln-remediation-pipeline](https://github.com/{repo}/tree/master/.github/vuln-remediation)_",
    ])
